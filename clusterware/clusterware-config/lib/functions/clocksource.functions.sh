...
cw_CLOCKSOURCE_export_dir=${cw_ROOT}/var/lib/clusterware-www/default/config
cw_CLOCKSOURCE_sym_link=${cw_CLOCKSOURCE_export_dir}/cluster-clocksource.rc
...
_clocksource_set_source() {
    ...
    if clocksource_is_master_node; then
        if [ -d "$cw_CLOCKSOURCE_export_dir" ]; then
